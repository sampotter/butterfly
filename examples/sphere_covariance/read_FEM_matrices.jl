...
for (i, r) in enumerate(rs)
    ...
    #     ]) / L2C
    nf = min(r, 50)
    mb = [r-nf+1:r ones(nf)] \ Lam[r-nf+1:r]
    ls = mb[1]*(1:length(Lam)) .+ mb[2]
    # pl = plot(1:r, Lam[1:r], marker=1, markerstrokewidth=0, label="")
    # plot!(pl, 1:r, ls[1:r], marker=1, markerstrokewidth=0, label="")
    # display(pl)
    errs[2,i] = norm(g.(ls[1+r:end]).^2) / normC
end
